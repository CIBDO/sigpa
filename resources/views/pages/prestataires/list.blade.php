                 ...
                 <div class="modal-dialog">
                     <div class="modal-content">
                         <div class="modal-header">
                             <h5 class="modal-title" id="ajouterMarqueModalLabel">Ajouter un prestataire</h5>
                             <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                         </div>
                         <div class="modal-body">
                             <form action="{{ route('prestataires.store') }}" method="post" id="ajouterMarqueForm">
                                 @csrf
             ...
             <script>
                 document.addEventListener("DOMContentLoaded", function() {
                     $("#ajouterMarqueForm").submit(function(event) {
                         var nomMarque = $("input[name='nom_prestataire']").val();
                         if (!nomMarque.trim()) {
                             alert("Veuillez entrer le nom du prestataire.");
                    ...
                    <tr class="odd">
                        ...
                        <td>
                           ...
                           {{--  <a class="me-3" href="{{route('marques.detail', ['marque' => $marque->id_marque])}}">
                            ...
                            <a class="me-3" href="{{route('prestataires.edit', ['prestataire' => $prestataire->id_prestataire])}}">
                                <img src="{{asset('assets/img/icons/edit.svg')}}" alt="img">
                            </a>
                            <a href="{{ route('prestataires.destroy', ['prestataire' => $prestataire->id_prestataire]) }}" class="" onclick="event.preventDefault(); if(confirm('Êtes-vous sûr de vouloir supprimer ce prestataire?'))
                                       document.getElementById('delete-marque-form-{{$prestataire->id_prestataire}}').submit();">
